...
contract QueryAccountCaller {
    ...
    function queryLength(uint256 solana_address) external returns (bool, uint256) {
        (bool success, uint256 result) = QueryAccount.length(solana_address);
        return (success, result);
    }

    function queryLamports(uint256 solana_address) external returns (bool, uint256) {
        (bool success, uint256 result) = QueryAccount.lamports(solana_address);
        return (success, result);
    }

    function queryExecutable(uint256 solana_address) external returns (bool, bool) {
        (bool success, bool result) = QueryAccount.executable(solana_address);
        return (success, result);
    }

    function queryRentEpoch(uint256 solana_address) external returns (bool, uint256) {
