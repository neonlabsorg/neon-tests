...
class SolanaClient(solana.rpc.api.Client):
    ...
    def request_airdrop(self, pubkey: Pubkey, lamports: int, **kwargs) -> RequestAirdropResp:
        airdrop_resp = None
        for _ in range(5):
...
def make_wsol(amount, solana_wallet, associated_address):
    ...
    tx.add(sp.transfer(sp.TransferParams(from_pubkey=solana_wallet, to_pubkey=associated_address, lamports=amount)))
