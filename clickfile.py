...
@click.option("-u", "--users", default=None, help="Number of users (will be generated before load test run)")
@click.option("-b", "--balance", default=None, help="Initial balance of each user in Neon")
@click.option("-a", "--bank_account", default=None, help="Bank account address")
@catch_traceback
def run(network, script, users, balance, bank_account):
    ...
    accounts = {}
    web3_client = web3client.NeonChainWeb3Client(proxy_url=config[network]['proxy_url'])
