...
def test_get_storage_at(neon_api_client_client, hello_world_contract):
    storage = neon_api_client_client.get_storage_at(hello_world_contract.eth_address.hex())["value"]
    zero_array = [0 for _ in range(31)]
    assert storage == zero_array + [5]

    storage = neon_api_client_client.get_storage_at(hello_world_contract.eth_address.hex(), index="0x2")["value"]
    assert storage == zero_array + [0]


def test_get_balance(neon_api_client_client, session_user, evm_loader):
    result = neon_api_client_client.get_balance(session_user.eth_address.hex())["value"]
...
def test_emulate_transfer(neon_api_client_client, second_session_user, session_user, account_info):
    result = neon_api_client_client.emulate(
...
def test_emulate_contract_deploy(neon_api_client_client, session_user):
    contract_code = get_contract_bin("hello_world")
    result = neon_api_client_client.emulate(session_user.eth_address.hex(), contract=None, data=contract_code)
    assert result["exit_status"] == "succeed", f"The 'exit_status' field is not succeed. Result: {result}"
    assert result["steps_executed"] > 100, f"Steps executed amount is wrong. Result: {result}"
    assert result["used_gas"] > 0, f"Used gas is less than 0. Result: {result}"


def test_emulate_call_contract_function(neon_api_client_client, session_user, hello_world_contract):
