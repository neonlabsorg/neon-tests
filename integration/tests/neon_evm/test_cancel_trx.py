...
class TestCancelTrx:
    ...
    def test_try_to_cancel_trx_after_timeout_expiration(
        ...
        check_holder_account_tag(storage_account, FINALIZED_STORAGE_ACCOUNT_INFO_LAYOUT, TAG_ACTIVE_STATE)
        initial_slot = sol_client.get_slot().value
    ...
    def test_try_to_cancel_trx_after_timeout_expiration_with_continue(
        ...
        """EVM can cancel transaction and finalize storage account only after last activity"""
        signed_tx = make_contract_call_trx(
            evm_loader, user_account, rw_lock_contract, "unchange_storage(uint8,uint8)", [1, 1]
        )
        ...
        while True:
            current_slot = sol_client.get_slot().value
            print(f"Initial slot: {initial_slot}; current slot: {current_slot} -> Diff: {current_slot - initial_slot}")
            if current_slot - initial_slot > 50:
                break
            if time.time() - initial_time > 150:
                assert current_slot - initial_slot > 100, "Timeout expired, less than 100 slots in 150 seconds"
            time.sleep(0.4)

        # Executing one more step to update lastUsedSlot
        evm_loader.send_transaction_step_from_instruction(
            operator_keypair,
            operator_balance,
            treasury_pool,
        ...
        updated_slot = sol_client.get_slot().value
        initial_time = time.time()  # used for exiting in emergency case
        ...
        # Attempt to cancel transaction after initial timeout expiration
        trx = Transaction()
        trx.add(
            make_Cancel(
                evm_loader.loader_id,
        ...
        while True:
            current_slot = sol_client.get_slot().value
            print(f"Updated slot: {updated_slot}; current slot: {current_slot} -> Diff: {current_slot - updated_slot}")
            if current_slot - updated_slot > 100:
                break
            if time.time() - initial_time > 150:
                assert current_slot - initial_slot > 100, "Timeout expired, less than 100 slots in 150 seconds"
            time.sleep(0.4)

        # Attempt to cancel transaction after updated timeout expiration
        trx = Transaction()
        trx.add(
            make_Cancel(
                evm_loader.loader_id,
