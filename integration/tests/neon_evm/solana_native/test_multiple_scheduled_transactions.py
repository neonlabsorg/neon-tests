...
class TestMultipleScheduledTrx:
    ...
    def test_2_depended_transactions_one_failed(
        ...
        emulate_result = neon_api_client.emulate(
            neon_user.neon_address.hex(), basic_contract.eth_address.hex(), data.hex(), chain_id=environment.network_ids['sol']
        )
    ...
    def test_tree_with_parallel_trx(
        self, evm_loader, neon_user, basic_contract, treasury_pool, holder_acc, operator_keypair, neon_api_client, environment
    ):
        nonce = evm_loader.get_neon_nonce(neon_user.neon_address, environment.network_ids['sol'])
        contract_data = 18
        data = abi.function_signature_to_4byte_selector("setNumber(uint256)") + eth_abi.encode(
    ...
    def test_deploy_and_call_contract(
        self, evm_loader, neon_user, neon_api_client, treasury_pool, operator_keypair, holder_acc, basic_contract, environment
    ):
        nonce = evm_loader.get_neon_nonce(neon_user.neon_address, environment.network_ids['sol'])
        contract_code = (
        ...
        emulate_deploy = neon_api_client.emulate(
            neon_user.neon_address.hex(), contract=None, data=contract_code, chain_id=environment.network_ids['sol']
        )
    ...
    def test_call_precompiled_by_scheduled_trx(
        self, evm_loader, neon_user, neon_api_client, treasury_pool, operator_keypair, holder_acc, spl_token_caller, environment
    ):
        nonce = evm_loader.get_neon_nonce(neon_user.neon_address, environment.network_ids['sol'])

        data = abi.function_signature_to_4byte_selector("initializeMint(uint8)") + eth_abi.encode(["uint8"], [9])
        emulate_result = neon_api_client.emulate(
            neon_user.neon_address.hex(), contract=spl_token_caller.eth_address.hex(), data=data, chain_id=environment.network_ids['sol']
        )
