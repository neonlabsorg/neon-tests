...
class TestScheduledTrx:
    ...
    def test_neon_get_pending_scheduled_transaction_low_gas_price_status(
        self, block_timestamp_contract, web3_client_sol, neon_user, treasury_pool, evm_loader, event_caller_contract
    ):
        contract, _ = block_timestamp_contract
        ...
        trx_estimate_obj = ScheduledTrxEstimateRequest(neon_user.checksum_address, contract.address, call_data)
        estimate_result = web3_client_sol.estimate_scheduled(neon_user.solana_account.pubkey(), [trx_estimate_obj])

        tx_test = ScheduledTransaction.from_estimate_result(0, trx_estimate_obj, estimate_result)

        # gas_limit = 200000
        gas_limit = tx_test.gas_limit - 100000
        base_fee_per_gas = web3_client_sol.base_fee_per_gas()
        max_priority_fee_per_gas = 2500000000
        max_fee_per_gas = base_fee_per_gas * 2 + max_priority_fee_per_gas
        ...
        pending_trx = web3_client_sol.get_pending_transactions(neon_user.checksum_address)
        assert pending_trx["0x0"][0]["status"] == "LowGasPrice"
    ...
    def test_neon_get_pending_scheduled_transaction_no_sols(
        self, web3_client_sol, neon_user_no_sols, common_contract, evm_loader, treasury_pool, pytestconfig
    ):
        ...
        estimate_result = web3_client_sol.estimate_scheduled(
            neon_user_no_sols.solana_account.pubkey(), [trx_estimate_obj]
        )

        tx = ScheduledTransaction.from_estimate_result(0, trx_estimate_obj, estimate_result)

        evm_loader.create_tree_account(
            neon_user_no_sols, treasury_pool, tx.encode(), wSOL["address_spl"], chain_id=evm_loader.sol_chain_id
        )
        web3_client_sol.send_all_scheduled_transactions([tx])
        pending_trx = web3_client_sol.get_pending_transactions(neon_user_no_sols.checksum_address)
    ...
    def test_multiple_scheduled_trx_with_failed_trx_wait_parent(
        ...
        for i in range(3):
            trx_estimate_obj_list.append(
                ScheduledTrxEstimateRequest(neon_user.checksum_address, counter_contract.address, data)
            )
        estimate_result = web3_client_sol.estimate_scheduled(neon_user.solana_account.pubkey(), trx_estimate_obj_list)
        trxs = []
        for i in range(3):
