...
class TestScheduledTrx:
    ...
    def test_neon_get_pending_scheduled_transaction_no_balance(
    ...
    def test_send_simple_single_trx_a(
        self, web3_client_sol, neon_user_no_sols, common_contract, evm_loader, treasury_pool, pytestconfig
    ):
        balance = evm_loader.get_solana_balance(neon_user_no_sols.solana_account.pubkey())
        amount = 9214000
        if pytestconfig.getoption("--network") != "mainnet":
            if balance < amount * LAMPORT_PER_SOL:
                evm_loader.request_airdrop(neon_user_no_sols.solana_account.pubkey(), amount, commitment=Confirmed)
