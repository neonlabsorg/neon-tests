...
class PostgresTestResultsHandler:
    ...
    def get_previous_tag(self, repo: str, branch: str, tag: str) -> str:
        ...
        sorted_tags = sorted([t[0] for t in tags if t[0] is not None], key=lambda t: LooseVersion(t), reverse=True)

        latest_tag = LooseVersion(tag)
        previous_tag = LooseVersion("v0")

        for tag_ in sorted_tags:
            next_tag = LooseVersion(tag_)
    ...
    def get_historical_data(
        ...
        previous_tag = tag if tag is None else self.get_previous_tag(repo=repo, branch=previous_branch, tag=tag)
