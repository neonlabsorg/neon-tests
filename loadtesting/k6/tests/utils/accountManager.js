...
export function createFundedAccount(client, amount, bankAccount) {
    var accountKey = wallet.generateKey();
    if (bankAccount != null) {
        fundAccountFromBank(client, bankAccount, accountKey.address, amount);
    } else {
        fundAccountFromFaucet(client, accountKey.address, amount);
    };
...
function requestFaucet(address, amount) {
    ...
    console.log('Faucet request: ' + faucetUrl);
