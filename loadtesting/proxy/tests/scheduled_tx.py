...
def prepare_one_contract_for_scheduled_trx(environment: env.Environment, **kwargs):
    ...
    environment.contract_info["recipients"] = []
...
class ScheduledTxsIndependentTasksSet(BaseScheduledTxTaskSet):
...
class ScheduledTxsDependentTasksSet(BaseScheduledTxTaskSet):
    ...
    def task_send_dependent_scheduled_tx(self):
        ...
        data_0 = data_1 = decode_function_signature(
            ...
            [
                self.user.environment.contract_info["erc20_owner_address"],
                self.neon_account.checksum_address,
                top_up_in_trx,
            ],
        )
...
class ScheduledTxsTransferToDifferentUsersTasksSet(BaseScheduledTxTaskSet):
